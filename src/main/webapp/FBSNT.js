...
var tFinal="";
var filterOnOff=0;
var CTOptions={width: "100%",
		height: "490px",
		scrolling: "yes"};
var CTOptionsPopUp={width: "100%",
		height: "95%",
		scrolling: "yes"};
...
"selects":[{"aggregation":"NONE","column":"A.AreaCode","alias":"AreaCode"},
	{"aggregation":"NONE","column":"A.AreaName"+FAOSTATFBS.lang,"alias":"AreaName"+FAOSTATFBS.lang},
...
"wheres":[
		...
		"ins" : []}
		 ...
		 "orderBys" :[{"column" : "A.AreaLevel"},{"column" : "A.AreaName"+FAOSTATFBS.lang}]
		,"limit" : null,
		 "query" : "NONE",
		 "frequency" : "NONE"
...
var param={
"selects":[{"aggregation":"NONE","column":"A.ItemCode","alias":"ItemCode"},	{"aggregation":"NONE","column":"A.ItemName"+FAOSTATFBS.lang,"alias":"ItemName"+FAOSTATFBS.lang}],
"froms":[
...
"wheres":[
		...
		"ins" : []}
		 ...
		 "orderBys" :[{"column" : "A.ItemName"+FAOSTATFBS.lang}]
		,"limit" : null,
		 "query" : "NONE",
		 "frequency" : "NONE"
};
...
,	{"aggregation":"NONE","column":"AreaName"+FAOSTATFBS.lang,"alias":"AreaName"+FAOSTATFBS.lang}
,	{"aggregation":"NONE","column":"ItemCode","alias":"ItemCode"}
,	{"aggregation":"NONE","column":"ItemName"+FAOSTATFBS.lang,"alias":"ItemName"+FAOSTATFBS.lang}
,	{"aggregation":"NONE","column":"ElementCode","alias":"ElementCode"}
,	{"aggregation":"NONE","column":"ElementName"+FAOSTATFBS.lang,"alias":"ElementName"+FAOSTATFBS.lang}
...
cell.setAttribute("id","thTitle");
 ...
 //  row.style.height="40px";
	...
	mybutton.onclick=function(){FAOSTATFBS.cache('myFilter');};
	//mybutton.onclick=function(){FAOSTATFBS.cache('myFilter');};
	//mybutton.addEventListener("click", FAOSTATFBS.cache('myFilter'), false); 
	
		mybutton.setAttribute("class","myButton");
	mybutton.innerHTML="<img src=\"/home/faostat-download-js/images/filter.png\" /><p class=\"up\">"+FAOSTATFBS.traduction[FAOSTATFBS.lang].filterOff+"</p>";
	
	if (navigator.appVersion.indexOf("MSIE 7.") != -1){}else{cell.appendChild(mybutton); }
	

	var myA=document.createElement("div");

	myA.setAttribute("onClick","FAOSTATFBS.toExcel()");
	myA.onclick=function(){FAOSTATFBS.toExcel()};
	myA.setAttribute("id","btnExcel");
	myA.innerHTML="<img src=\"/home/faostat-download-js/images/export.png\" /><p class=\"up\">"+FAOSTATFBS.traduction[FAOSTATFBS.lang].exportdata+"</p>";
	...
	myA.innerHTML=	"<img src=\"/home/faostat-download-js/images/full.png\"/><p class=\"up\">"+FAOSTATFBS.traduction[FAOSTATFBS.lang].fullscreen+"</p>";
...
cell.onclick=function(){showData();};
cell.appendChild(txtCell); 
row.appendChild(cell); 
...
var row = document.createElement("tr");
if(document.getElementById("showDomestic").checked){FAOSTATFBS.headert=FAOSTATFBS.header;}else{FAOSTATFBS.headert=FAOSTATFBS.headerNoDom;}
for(var i=0;i<FAOSTATFBS.headert.length;i++)
{
var cell = document.createElement("th");
cell.style.cursor="pointer";
var ii=i;
cell.setAttribute("OnClick","FAOSTATFBS.reTri("+ii+")");
cell.onclick=function(){FAOSTATFBS.reTri(ii)};
...
myFilter3.innerHTML="";
 ...
 for(var i=1;i<myJson.length;i++)
 {
	 if(curItem!=myJson[i][3])
		...
		myfil.onclick=function(){FAOSTATFBS.cache('myTr_'+nl);}
...
if(myJson[i][0]==15){		
 ...
 if(document.getElementById("showDomestic").checked){
 if(FAOSTATFBS.myIndex[myJson[i][5]]>0){ myLine[FAOSTATFBS.myIndex[myJson[i][5]]]=parseFloat(myJson[i][8]); }
 }
 else{ if(FAOSTATFBS.myIndexNoDom[myJson[i][5]]>0){ myLine[FAOSTATFBS.myIndexNoDom[myJson[i][5]]]=parseFloat(myJson[i][8]); } }
 }
  nlt=nl-1;
...
totChroma=$("#resultTable").chromatable(CTOptions);	
		...
		param+="&select=ItemLevel,AreaCode,AreaName"+FAOSTATFBS.lang+",ItemCode,ItemName"+FAOSTATFBS.lang+",ElementCode,ElementName"+FAOSTATFBS.lang+",Year,Value,Flag";
		param+="&from=vFBSItem";
...
var cell = document.createElement("th");
	...
	mybutton.innerHTML="<img src=\"/home/faostat-download-js/images/filter.png\" /><p class=\"up\" id=\"pup\">"+FAOSTATFBS.traduction[FAOSTATFBS.lang].filterOff+"</p>";
...
mydiv.appendChild(myFilter3);
cell.appendChild(mydiv);
if (navigator.appVersion.indexOf("MSIE 7.") != -1){}else{row.appendChild(cell);}
tblH.appendChild(row);
/*FIN FILTER*/
var row = document.createElement("tr");
var cell = document.createElement("th");
...
if(document.getElementById("showDomestic").checked){FAOSTATFBS.headert=FAOSTATFBS.header;}else{FAOSTATFBS.headert=FAOSTATFBS.headerNoDom;}
console.log("headerT"+FAOSTATFBS.headert.length);
for(var i=0;i<FAOSTATFBS.headert.length;i++)
{
var cell = document.createElement("th");
cell.style.cursor="pointer";
...
var ii=ioo;
cell.setAttribute('name',ii);
cell.setAttribute('OnClick',"FAOSTATFBS.reTri("+ii+")");

cell.onclick=function(){FAOSTATFBS.reTri(this.name)};
cell.appendChild(txtCell);
row.appendChild(cell);
}
tblH.appendChild(row);
...
myFilter3.innerHTML="";
 ...
 if(myLine.length>0){nlt=nl-2;FAOSTATFBS.writeTR(tblB,myLine,"myTr_"+nlt);}
 myLine=[];
 }
 ...
 if(FAOSTATFBS.myIndexNoDom[myJson[i][5]]>0){ myLine[FAOSTATFBS.myIndexNoDom[myJson[i][5]]]=parseFloat(myJson[i][8]); }
 }
 }
   nlt=nl-1;
FAOSTATFBS.writeTR(tblB,myLine,"myTr_"+nlt);//derniere Ligne
...
b1.onclick=function(){FAOSTATFBS.refAll(0);}
...
b1.onclick=function(){FAOSTATFBS.refNone(0)};
myFilter2Button.appendChild(b1);
var b1=document.createElement("input");
b1.setAttribute("type","button");
...
b1.onclick=function(){FAOSTATFBS.refRev(0)};
...
b1.setAttribute("onclick","FAOSTATFBS.refAll(1)");
 b1.onclick=function(){FAOSTATFBS.refAll(1)};
...
b1.onclick=function(){FAOSTATFBS.refNone(1)};
myFilter3Button.appendChild(b1);
var b1=document.createElement("input");
b1.setAttribute("type","button");
b1.setAttribute("value",FAOSTATFBS.traduction[FAOSTATFBS.lang].reverse);
...
b1.onclick=function(){FAOSTATFBS.refRev(1);};
myFilter3Button.appendChild(b1);
...
totChroma.resize();	
 ...
 FAOSTATFBS.reTri=function(io){
 console.log("IO"+io);
if(io==tri){asc++;if(asc==2){asc=0;}}
tri=io;
