...
$.post( "/wds/rest/table/json", FAOSTATOLAP2.queryParams).done(function( data ) 
	...
	$("#newOlap").pivotUI(data, FAOSTATOLAP2.options,true);
	});
...
sum2: function(sigfig, scaler) {
      ...
      return function(_arg) {
	   ...
	   return function() {
          return {
			...
			//console.log(j+" "+this.sum[j]+" "+record[_arg[j]]);
			
			  this.sum[j]+=parseFloat(record[_arg[j]].replace(",",""));
			  //console.log(j+" "+this.sum[j]);
			  }
			  else if(_arg[j]=="Unit"){
			 if(this.sum[j]=="_" || this.sum[j]=="("+record[_arg[j]]+")" ){
			  this.sum[j]="("+record[_arg[j]]+")";}
			  else{
			  console.log(this.sum[j]+" :"+record[_arg[j]]);
			  this.sum[0]=NaN;
			  this.sum[j]="nan";}
			  }
			  }
  ...
  aggregators = {
   ...
   sum2: aggregatorTemplates.sum2(3),
     sumUnit:aggregatorTemplates.sumUnit(3),
    count: function() {
      return function() {
        return {
          count: 0,
  ...
  */
  $.fn.pivotUI = function(input, inputOpts, overwrite) {
    ...
    this.find(".pvtAxisContainer").sortable({
	  ...
	  console.log(e)
	  var my_id=e.toElement.id.split("_")[1];
	  if(e.target.id!="unused")
	  {
	  for(var kop in FAOSTATOLAP2.options.linkedAttributes)
	  {console.log(kop)
	  console.log(FAOSTATOLAP2.options.linkedAttributes[kop]);
	   try{
	  if(FAOSTATOLAP2.options.linkedAttributes[kop].indexOf(my_id)!=-1)
	  {
	   for(var kop2 in FAOSTATOLAP2.options.linkedAttributes[kop])
	  {
	  console.log(kop+"\\"+kop2);
	  console.log(FAOSTATOLAP2.options.linkedAttributes);
	  try{
	   $("#"+e.target.id).append($("#axis_"+FAOSTATOLAP2.options.linkedAttributes[kop][kop2]));
	  }catch(e){}
	  }
	  break;
	  }
